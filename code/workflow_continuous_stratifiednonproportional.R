...
# OKAY! So say we want this to be non-proportional. Easiest way to do that (I think) is to reorder
# the proportional-by-area values
reorder_seed <- simulation_seed * 109
set.seed(reorder_seed)
reordered_indices <- sample(x = 1:n_strata,
                            size = n_strata)
while (identical(reordered_indices, 1:n_strata)) {
  reorder_seed <- reorder_seed + 10000
  reordered_indices <- sample(x = 1:n_strata,
                              size = n_strata)
}
strata_polygons$n_points <- strata_polygons$n_points[reordered_indices]
