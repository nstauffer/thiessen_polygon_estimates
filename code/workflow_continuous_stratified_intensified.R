...
#### Generate intensification polygon  ------------------------------------------------
int_polygon <- aoi_gen(xmax = raster_ncol,
                       xmin = 0,
                       ymax = raster_nrow,
                       ymin = 0,
...
strata_polygons$n_points <- ceiling(strata_polygons$proportional_area * n_aoi_sample_points)
...
n_excess_points <- sum(strata_polygons$n_points) - n_aoi_sample_points
...
sample_points_list_aoi <- lapply(X = sample_seeds,
...
##### Create points within the intensification ####
sample_points_list_int <- lapply(X = sample_seeds,
                                 frame = int_polygon,
...
sample_points_list <- mapply(X = sample_points_list_aoi,
                             Y = sample_points_list_int,
                             SIMPLIFY = FALSE,
                             FUN = function(X, Y){
                               rbind(X, Y)
                             })
...
# These'll be special because we have an intensification
strata_polygons$uid <- strata_polygons$stratum_id
int_polygon$uid <- "intensification"

all_polygons <- rbind(strata_polygons[, c("uid")],
                      int_polygon[, c("uid")])

wgtcat_polygons <- wgtcat_gen(polygons = all_polygons,
                              aoi_index = NULL)
