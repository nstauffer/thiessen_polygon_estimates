...
output_gdb_filename <- "ecoregional_pj_analysis_20250801.gdb"


##### Parameters ---------------------------------------------------------------
analysis_name <- "ecoregional_pjcover"
file_date <- "20250801"
...
# AIM defaults to 80%
conf <- 80
continuous_indicators <- "pj_cover"
...
pj_cover_classes <- list("1-9%" = c(min = 0.00001,
                                    max = 10),
                         "10-20%" = c(min = 10,
                                      max = 20),
                         "No PJ" = c(min = 0,
...
if (use_ldc) {
  ...
  if ("PINTERCEPT" %in% layers_lookup$layer_name & !is.null(input_data_list[["lmf"]][["headers"]])) {
    ...
    warning("Unable to find either a PINTERCEPT table or POINTS feature in the geodatabase. No AIM points will be used.")
    input_data_list[["aim"]][["lpi_detail"]] <- NULL
  }
...
##### Prepping data ------------------------------------------------------------
# Add in a PJ variable
lpi_data <- dplyr::mutate(.data = lpi_data,
                          pj = dplyr::case_when(code %in% pj_identifiers ~ "pj_cover",
                                                .default = NA))

##### Calculating indicators ---------------------------------------------------
# We'll calculate any hit cover (a pin drop will count towards cover if any
# layer record at that location is associated with a pinyon or juniper species.)
pj_cover <- terradactyl::pct_cover(lpi_tall = lpi_data,
                                   tall = FALSE,
                                   hit = "any",
                                   by_line = FALSE,
                                   indicator_variables = c("pj_cover"),
                                   digits = 1,
                                   verbose = TRUE)

##### Classifying by PJ cover --------------------------------------------------
...
for (current_category in names(pj_cover_classes)) {
  pj_cover <- dplyr::mutate(.data = pj_cover,
                            # We stated above that the min is inclusive but the
                            # max is not!
                            meets_min = PJ >= pj_cover_classes[[current_category]]["min"],
                            meets_max = PJ < pj_cover_classes[[current_category]]["max"],
                            qualifies = meets_min & meets_max)
  pj_cover[["cover_category"]][pj_cover$qualifies] <- current_category
  pj_cover <- dplyr::select(.data = pj_cover,
...
analysis_list <- lapply(X = weighting_list,
                        points = sf::st_drop_geometry(pj_cover) |>
...
tpoly_summaries <- dplyr::filter(.data = tpoly_summaries,
  ...
  dplyr::mutate(.data = _,
                ...
                mean_estimate = dplyr::case_when(indicator == "pj_cover" ~ mean_estimate / 100,
...
# tpoly_summaries_continuous <- dplyr::filter(.data = tpoly_summaries,
#                                             indicator == "pj_cover")
...
output_results_wide <- dplyr::select(.data = tpoly_summaries,
  ...
  dplyr::mutate(.data = _,
                ...
                indicator = dplyr::case_when(!stringr::str_detect(string = indicator,
                                                                  pattern = "cover") ~ paste0("pj_cover_",
                                                                                             indicator),
                                             .default = indicator),
  ...
  tidyr::pivot_wider(data = _,
                     ...
                     values_from = value) |>
  dplyr::rename(.data = _,
                "PJ cover (percent)" = "pj_cover (proportion)") |>
  dplyr::mutate(.data = _,
                `PJ cover (percent)` = `PJ cover (percent)` * 100)
...
write.csv(x = analysis_results,
          file = paste0(output_path, "/", analysis_name, "_", "_results_", file_date, ".csv"),
          row.names = FALSE)

write.csv(x = tpoly_summaries,
          file = paste0(output_path, "/", analysis_name, "_", "_tpoly_summaries_", file_date, ".csv"),
          row.names = FALSE)
